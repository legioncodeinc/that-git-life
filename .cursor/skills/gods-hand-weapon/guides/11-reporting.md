...
The final report is `gods-hand`'s only output to the caller. The tracking-file mutations and on-disk artifacts are the durable products; the report is the human-readable receipt. Make it concise, complete, and structured so an orchestrator (or human) can decide what to do next.
...
- `Forged Angel 001|nextjs-guardian. Pipeline complete.`
...
| File | Before | After |
|---|---|---|
| `proposed-angels-queue.md` | Top row was `NNN\|guardian-name` | Row removed; new top row is `MMM\|next-guardian-name` (or "queue empty") |
| `proposed-angels-in-process.md` | Empty | Row was appended then removed; file is now empty |
| `proposed-angels-completed.md` | Empty / N rows | Row appended; file now has N+1 rows |
...
| 3 | angel-creator | 0:01:02 | OK |
| 4 | god-registrar | 0:00:18 | OK |
...
- Other items that warrant human follow-up but do not invalidate the Angel.
...
- `Cycle complete. Invoke gods-hand again to process the next queue entry, or stop here.`
- `Cycle paused at Phase 2. Resolve the failure (see in-process row marker) and re-invoke.`
- `Queue empty. No further gods-hand invocations needed until big-bang-space queues more rows.`

This section is the EXPLICIT statement of `gods-hand`'s stop rule. By writing the next-steps line, the Angel makes clear that it does not auto-advance. The orchestrator decides.
...
This is the signal to the orchestrator that the run is over. Do NOT continue with chat after this line. Do NOT recursively invoke `gods-hand`. Do NOT pick up the next queue entry.

This line is load-bearing per `guides/00-principles.md` non-negotiable #7 ("Stop after god-registrar"). Without it, the orchestrator might assume `gods-hand` is still working and wait indefinitely.

## A note on context compaction

If `gods-hand` is invoked for a deep-tier cycle that takes hours, context compaction may erase the early steps of the run before close-out. The final report becomes hard to assemble.
