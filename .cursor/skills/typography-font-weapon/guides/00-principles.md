...
> **Open question (Q3 from research):** Does your project prioritize brand consistency (use `swap`) or Core Web Vitals (use `optional`)? The weapon defaults to `optional` + preload for `next/font` projects and `swap` + `size-adjust` for self-hosted fonts. Override this in the hosting guide for your project type.
...
1. **Token-first:** Every size, weight, and family reference in component code must route through a CSS custom property. Raw values create drift the guardian cannot audit.
