...
6. **The DB-side concern (locks, indexes, constraints, partitioning) hands off to `db-guardian`.** This guide is about the Django mechanics.
...
- **`scripts/audit-applied-migrations.py`** in this Weapon — verify no edits to migrations already in `git log` of a deployed environment.
...
## Handoff to db-guardian

- Postgres-side concerns (lock modes, partitioning, index strategy, vacuum / analyze, replication considerations) → `db-guardian`.
- Django-side migration mechanics (the patterns above, `RunPython` shape, `RunSQL` correctness, `--check` in CI) → this Weapon.
