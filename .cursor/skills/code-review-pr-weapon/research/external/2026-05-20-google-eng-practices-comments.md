...
- **Severity label system** is the authoritative precedent for the Angel's three-tier comment taxonomy. The Angel's taxonomy (`blocker` / `suggestion` / `thought`) should explicitly acknowledge this Google origin.
...
- **Contradiction to resolve**: Google's "FYI:" label has no direct equivalent in the three-tier system. The Angel could add a fourth "educational" tier, or collapse FYI into "thought" (the lightest tier). The weapon-forge author should decide.
