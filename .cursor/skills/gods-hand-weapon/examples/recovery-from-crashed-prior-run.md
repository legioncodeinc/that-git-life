...
What `gods-hand` does when invoked while `proposed-angels-in-process.md` has an orphaned row from a previously-crashed cycle. This is failure mode F0.2 in `guides/10-failure-modes.md`.

## Starting state

The user invokes `gods-hand` expecting a normal cycle:

> "Run the pipeline."

The four tracking files are:

- `proposed-angels-queue.md`: top row `006|shadcn-component-library-guardian` (231 - 1 = 230 in body; one was already processed in a prior cycle).
- `proposed-angels-in-process.md`: one row: `005|vite-guardian|failed:weapon-forge|2026-05-19`.
- `proposed-angels-completed.md`: 1 row from a happy-path prior cycle: `001|nextjs-guardian|completed|2026-05-18|...`.
...
`gods-hand` reads `proposed-angels-queue.md`. Top row of body would be `006|shadcn-component-library-guardian`.

But BEFORE proceeding, `gods-hand` always checks `proposed-angels-in-process.md` per the F0.2 pre-flight check.

## F0.2 trigger fires

`gods-hand` reads `proposed-angels-in-process.md` and finds:
...
`gods-hand` STOPS. It does NOT pick row `006`. It does NOT delete the orphan. It does NOT touch any tracking file.

## Diagnosis emitted to the caller

`gods-hand` emits a structured message explaining the orphan:

```markdown
## gods-hand pre-flight check failed

The factory is not in a clean state. The in-process tracking file has an orphaned row from a prior run:

- File: `ai-tools/proposed-angels-in-process.md`
- Row: `005|vite-guardian|failed:weapon-forge|2026-05-19`
- Diagnosis: A prior cycle started but failed at Phase 2 (weapon-forge). The Command Brief and research folder exist on disk; the Weapon SKILL.md, Angel file, and God's roster row do NOT.
...
Manually edit:
- `ai-tools/proposed-angels-in-process.md`: delete the row.
...
If row 005 should not be forged at all (e.g., the Angel proposal is being retired):

- Delete the row from `ai-tools/proposed-angels-in-process.md`.
...
gods-hand stopped. Awaiting human decision.
```

## What this example demonstrates

- The F0.2 pre-flight check ALWAYS runs before any other work.
- `gods-hand` does NOT auto-recover. Each recovery option requires human judgment because the right answer depends on context (was the failure transient? is the brief still relevant? has the domain shifted?).
- The orphan row carries enough information (`|failed:<phase>|<date>`) to make the diagnosis without `gods-hand` having to inspect the artifacts on disk.
- The stop line is honored: no automatic retry, no advancing to the next queue row, no silent rollback.

## What `gods-hand` does NOT do
...
In practice, Option A (retry from the failed phase) is the most common choice for transient failures (model timeout, network error). Option B is chosen when the brief needs to be re-written. Option C is rare; it usually accompanies a separate decision to retire the Angel from the backlog.

After the human chooses an option and applies the manual edits (Option A skips manual editing; the next `gods-hand` invocation is instructed to retry), the next `gods-hand` invocation sees an empty in-process file and proceeds normally.
