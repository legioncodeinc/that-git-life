...
## React side (sketch — full React work hands off to react-guardian)
...
- **Auth provider choice (Clerk / WorkOS / Auth0 / Supabase Auth)** — `auth-guardian`.
- **React component / state / data-fetching shape** — `react-guardian`.
- **Security audit (XSS surface, RBAC correctness, threat model)** — `security-guardian`.
