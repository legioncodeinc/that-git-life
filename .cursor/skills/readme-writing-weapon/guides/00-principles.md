...
This is the foundational constraint that drives every decision in this weapon:
...
Extraction threshold: 2,000 words — flag for docs-site extraction route to `library-guardian`.
...
These conditions require escalating outside this weapon:

| Condition | Action |
|---|---|
| README exceeds 2,000 words | Flag extraction; hand off to `library-guardian` |
| User wants a full docs site | Hand off to `library-guardian` |
| CI badge pipeline needs setup | Hand off to `devops-guardian` |
| Python `.rst` README | Hand off to `python-guardian` |
