...
- The `disable-model-invocation: true` flag is interesting for `gods-hand-weapon` specifically: should the skill auto-load when `gods-hand` reasons about its own task, or only when explicitly slash-invoked? Recommendation for `weapon-forge`: leave it as default (`false`) so `gods-hand`'s body can reference the weapon via natural-language directives like "load the gods-hand-weapon skill" without needing to type a slash command.
