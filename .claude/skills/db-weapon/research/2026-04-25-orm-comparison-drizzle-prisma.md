...
## N+1 risk patterns (cross-cuts react-guardian)

The most common N+1 in any ORM is "`include` looks innocent in the query, but each row fans out to its own SELECT". db-guardian's responsibility is to **flag the risk in the schema/query layer** and hand off to `react-guardian` for the data-layer remediation (RSC vs route loader vs TanStack Query batching).
