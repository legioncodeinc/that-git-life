...
The operational core of `gods-hand`. This guide covers Steps 1 and 2 of the Command Brief's ACTION list: picking the top queue row and locking it into `in-process` BEFORE doing any phase work.

## Step 1: Pick the top row from the queue

Open `ai-tools/proposed-angels-queue.md`. The file has YAML frontmatter (lines 1-40 roughly) followed by a body of rows. Each row has the exact form:

```
NNN|guardian-name
```

where `NNN` is a zero-padded 3-digit position number and `guardian-name` is the kebab-case Angel name.

The top row is the lowest `NNN` still present. The queue is monotonic by `pickup_protocol`'s contract: positions are never reused and never inserted in the middle. `big-bang-space` only ever appends to the bottom.

Capture three things from the picked row:

1. `NNN` -- the position number (preserve the zero-padding).
2. `guardian-name` -- the Angel name as it appears in the row.
...
### Sub-step 2a: Delete the picked row from `proposed-angels-queue.md`
...
- Set `last_updated_by:` to `gods-hand`.
...
### Sub-step 2c: Append the row to `proposed-angels-in-process.md`

Append a single line to `ai-tools/proposed-angels-in-process.md`:
...
The research surfaced a stale section in `ai-tools/proposed-angels-queue.md`'s YAML frontmatter. The `pickup_protocol` text in the queue file describes a ONE-stage lifecycle (queue -> completed directly). The Command Brief and this guide specify a TWO-stage lifecycle (queue -> in-process -> completed).

`gods-hand` follows the TWO-stage lifecycle. Treat the queue file's `pickup_protocol` text as out-of-date documentation that needs a manual edit (suggested wording: replace "appends the row to `proposed-angels-completed.md` with a status marker and ISO date" with "appends the row to `proposed-angels-in-process.md`; the row moves to `proposed-angels-completed.md` only after all five phases succeed via `gods-hand`'s close-out step").
