...
- The weapon should include a concrete worked example: an OKR with the same Objective but three different KR formulations: sandbagged (will achieve with normal effort), aspirational-correct (will hit ~70% with stretch effort), and aspirational-impossible (will hit 0-30% because it requires events outside the team's control).
