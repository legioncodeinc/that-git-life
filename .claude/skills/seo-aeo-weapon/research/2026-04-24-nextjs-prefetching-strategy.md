...
Next.js App Router automatically prefetches in-viewport `<Link>` destinations during idle time. The resulting navigations feel instant. Tradeoff: aggressive prefetch can over-fetch on low-bandwidth devices or bloat the initial idle budget. The Weapon codifies when to tune this.
