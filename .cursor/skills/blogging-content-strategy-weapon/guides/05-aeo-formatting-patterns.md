...
**Technical implementation note:** The actual FAQPage schema markup (`application/ld+json`) is handled by `seo-aeo-guardian`, not by this Weapon. Your job is to write the question-answer pairs in this structure so they are ready for schema application.
...
- **Do not block AI crawlers.** If your `robots.txt` blocks `OAI-SearchBot`, `PerplexityBot`, `Claude-User`, or `ChatGPT-User`, you are blocking citation entirely. Technical implementation is handled by `seo-aeo-guardian`, but flag this as a handoff item.
...
- [ ] No AI crawlers blocked in robots.txt (flag to `seo-aeo-guardian` for verification).
