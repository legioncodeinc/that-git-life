...
Procedural arsenal for `product-tour-onboarding-ui-guardian`, the Angel that owns the in-app guided-experience layer.
...
Do NOT load for:
- Onboarding email sequences (out of scope — no Angel owns this yet).
- User authentication flows (auth-guardian).
- Design token system for tour tooltip styling (ux-ui-guardian).
- Analytics funnel instrumentation for tour events (posthog/mixpanel Angels).
...
- **Do not instrument analytics yourself.** Flag what needs tracking; route to the appropriate analytics Angel.
...
*Forged by weapon-forge from `product-tour-onboarding-ui-guardian-command-brief.md` and `research/`. Part of the Legion AI Tools Factory.*
