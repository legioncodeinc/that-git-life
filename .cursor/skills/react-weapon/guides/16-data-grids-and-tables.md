...
- **Token / spacing / typography of the table surface** → `ux-ui-guardian`.
- **Server-side row model API design (cursor pagination, filter pushdown)** → `db-guardian`.
- **License procurement (AG Grid, Handsontable, MUI X)** → `library-guardian` PRD.
