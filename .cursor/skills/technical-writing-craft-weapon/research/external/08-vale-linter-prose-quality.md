...
- `guides/06-docs-as-code-review.md` should reference Vale as the recommended CI lint layer for docs PRs, noting what Vale checks automatically vs. what the Angel checks manually.
- The answer to Command Brief Q1 (Vale + Diataxis ruleset): does not exist. Weapon-forge should document this explicitly to prevent a wild goose chase. Diataxis mode classification requires semantic judgment; automate voice/tense/capitalization with Vale, automate Diataxis with the Angel.
- A recommended Vale configuration for Legion projects could be documented in the Weapon as a sidebar or appendix: `Google` style + custom rules for Legion-specific vocabulary.
...
- Vale does not replace the Angel's review - it reduces the noise of trivial style issues so the Angel can focus on structural and clarity problems.
