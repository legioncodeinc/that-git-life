...
The security audit (provider choice, OAuth flow review, RBAC analysis, secret-rotation policy) goes to `security-guardian`. python-guardian's job is to ensure the baseline above is in place and `check --deploy` is clean.
