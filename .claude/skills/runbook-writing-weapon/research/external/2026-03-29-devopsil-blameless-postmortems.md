...
- The "write runbook" action item pattern gives weapon-forge a concrete handoff protocol: when a postmortem identifies a scenario with no runbook, the action item is `[Write runbook for <scenario>]` with a named owner and a due date.
...
- The monthly action item review cycle gives weapon-forge a recommended freshness cadence to pair with the quarterly game day cadence.
