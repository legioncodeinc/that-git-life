...
- CSP baseline (to be tightened by `security-guardian`)
...
- `Content-Security-Policy` baseline present (to be tightened by `security-guardian` before launch)
...
> CSP route: tighten via `security-guardian` before production. `'unsafe-inline'` is a baseline only.
...
- **R-2:** CSP `'unsafe-inline'` for scripts is broad. GA4's inline snippet requires it. Route CSP tightening through `security-guardian` to identify if `nonce`-based CSP is feasible given GA4 requirements.
