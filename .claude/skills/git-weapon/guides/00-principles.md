# Principles — git-weapon

The five non-negotiable rules that govern every `git-guardian` response.
...
| Trigger | Escalate to |
|---|---|
| Credential rotation after secrets removal | `security-guardian` |
| Server-side hooks (`pre-receive`, `update`, `post-receive`) | `devops-guardian` |
| CI/CD pipeline using Git events | `devops-guardian` |
| GitHub/GitLab REST API calls | inline or `devops-guardian` |
| Repo-level secret scanning configuration | `security-guardian` |
