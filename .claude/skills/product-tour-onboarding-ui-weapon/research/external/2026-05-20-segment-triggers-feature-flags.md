...
- The PostHog plugin integration with automatic feature flag sync is highly relevant for teams already using PostHog (common in the target audience). weapon-forge should include a concrete snippet for `@tour-kit/analytics` PostHog plugin setup.
- The "don't show again" persistence pattern is the most common implementation mistake — weapon-forge must place it as a "required companion" rule, not an optional enhancement, in `guides/04-segment-triggers.md`.
...
- Open question: userTourKit appears to be a relatively new library (v0.11.0, May 2026). weapon-forge should note it as an "emerging alternative" and verify production stability before recommending it over Driver.js/Shepherd.js for mission-critical onboarding.
