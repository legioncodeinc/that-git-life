...
For weapon-forge: the weapon should include a TCO calculator framework. Key cost drivers:
...
The Crisp vs Intercom comparison at scale is stark. The weapon should make this explicit with real numbers.
...
From research synthesis, the weapon should encode these questions as the pre-selection checklist:
...
- **guides/01-platform-selection.md**: The decision tree structure should be: Channel → Budget → AI readiness. The weapon should give a concrete recommendation ("Use Crisp" / "Use Intercom Early Stage" / "Use Pylon") rather than a comparison table. The command brief explicitly requires "a recommendation with a 2-sentence rationale, not just a comparison table."
- **Data export warning**: The command brief mandates surfacing the data-export discipline on every platform-selection call. The weapon should include a "before you sign" checklist with data export verification as item #1 — this is how teams avoid platform lock-in.
- **Drift handling**: The weapon should include a prominent warning at the top of the platform selection guide: "Drift is in gradual sunset as of March 2026. Do not use for new implementations."
- **Intercom pricing sensitivity**: Confirmed by research — the TCO model for Intercom with AI fees and seat costs is highly sensitive to conversation volume. The weapon must include the cost formula.
