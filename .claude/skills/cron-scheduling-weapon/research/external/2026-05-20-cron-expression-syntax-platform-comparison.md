...
Cron expression syntax varies meaningfully across platforms. POSIX 5-field is the universal baseline. Quartz 6/7-field adds seconds and optional year. Cloudflare extends the 5-field syntax with Quartz special characters (L, W, #). Vercel strips down the syntax (no named days/months, no combined DOM and DOW). GitHub Actions now supports IANA timezone strings as a field. This source synthesizes the official docs of each platform to produce the authoritative comparison table for the weapon's `guides/00-cron-expression-syntax.md`.
