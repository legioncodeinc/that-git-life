...
- The "completion rate is a vanity metric" thesis must be stated plainly in `guides/00-principles.md` (the principles guide) — it reframes the entire weapon's purpose from "build tours that get completed" to "build tours that drive activation."
...
- Open question: the article does not specify how to set up attribution tracking in PostHog or Amplitude specifically (it references them as options). weapon-forge should include a concrete PostHog funnel query example for the tour-to-activation funnel in `templates/tour-audit-report.md`.
