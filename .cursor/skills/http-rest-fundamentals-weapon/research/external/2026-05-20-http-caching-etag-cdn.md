...
- `guides/03-headers.md`: the caching strategy table by content type is weapon-forge-ready as a "recommended caching strategy" lookup table.
...
- The `no-store` directive for sensitive data (auth tokens, payment data) is a security-relevant caching finding — pairs with `security-guardian` handoff.
