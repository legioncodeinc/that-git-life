...
- **guides/03 (Sync Architecture):** This source is the primary reference for the entire sync guide. The four-loop architecture should be the top-level structure of the guide. The field ownership matrix table is a template the guardian should deliver as output.
...
- **templates/sync-state-schema.sql:** The reconciliation job needs a table tracking `last_successful_run` per `(tenant_id, provider, resource_type)`. This is the table the Command Brief wants weapon-forge to produce for db-guardian review.
