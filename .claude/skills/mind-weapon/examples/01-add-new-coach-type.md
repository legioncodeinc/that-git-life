...
mind-guardian's job: walk the contributor through the canonical add-coach checklist.
...
- **`asset-guardian`** — register `pricing_strategist` in the coach asset registry.
- **`db-guardian`** — confirm the `AiCoachConfig` index strategy supports the new coach type.
- **`library-guardian`** — author the PRD if this is being shipped as a feature.
