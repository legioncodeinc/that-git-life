...
## `pgvector` — storage decision (db-guardian's territory)

`pgvector` is the de facto standard Postgres extension for vector search. db-guardian picks:
...
db-guardian picks the column type, dimension, index family, and distance op. **Retrieval strategy** (top-k, hybrid dense+sparse, query expansion, reranking with cross-encoder or ColBERT, eval framework) is `ai-platform-guardian`'s.

When the user asks "should this be `ivfflat` or `hnsw`?", db-guardian answers. When the user asks "how should I chunk these PDFs and rerank the top-k?", db-guardian hands off.
...
- Vector hybrid search (use `pgvector` + a hybrid retrieval layer; hand to ai-platform-guardian).
...
- Hand off retrieval / RAG to `ai-platform-guardian`.
