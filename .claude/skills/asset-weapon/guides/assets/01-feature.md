...
- **library-guardian** — feature PRDs are numbered + placed at `library/requirements/features/feature-<###>-<title>/` + cross-linked by them.
- **ux-ui-guardian** — every feature that renders UI must reference a surface/control/display catalogued by you, but *visually specified* by ux-ui.
- **security-guardian** — any feature that touches auth, PII, or external integrations must route through a security review before `phase: shipped`.
- **quality-guardian** — verifies the feature's PRD vs implementation; does NOT verify registry linkage (that's your drift audit).

Note: example `prdRef` values throughout this Weapon (e.g., `FEA-XXX`, `BILL-007`) are illustrative placeholders. The deploying product substitutes its own feature PRD IDs.
