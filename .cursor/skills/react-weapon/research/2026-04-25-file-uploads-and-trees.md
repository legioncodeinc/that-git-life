...
- Storage backend (S3 / R2 / GCS), signed URLs, lifecycle policies handed off to `devops-guardian`.
- Per-upload auth scoping handed off to `security-guardian`.
- Image transforms / CDN delivery (Cloudinary, Imgix, unpic) referenced from the source doc but out of scope here — flagged for `library-guardian` PRD if needed.
