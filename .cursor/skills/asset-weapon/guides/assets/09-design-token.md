...
| `semanticRole` | `String?` | human | yes | human description — "primary brand color used on CTAs"; maintained by ux-ui-guardian |
...
- **Required annotations:** none in code; the generator derives `category` from the prefix (`--color-*` to `color`, `--radius-*` to `radius`, etc.). `semanticRole` must be filled by ux-ui-guardian via the admin UI.
...
- **ux-ui-guardian** owns `semanticRole` entirely. You never invent what a token means.
...
- [ ] `semanticRole` is filled by ux-ui-guardian (not by you)
- [ ] `tenantOverridable` defaults to `true` unless ux-ui-guardian opts out
