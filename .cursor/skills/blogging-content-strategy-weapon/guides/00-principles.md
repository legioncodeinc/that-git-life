...
The non-negotiables that govern every recommendation from `blogging-content-strategy-weapon`. These are not preferences; they are structural constraints derived from the research and from the Command Brief's SUBAGENT CRITICAL DIRECTIVES.
...
### 6. Hand off technical SEO decisions to `seo-aeo-guardian`

**Why:** This Weapon owns strategy and copy. Implementation details — robots meta tags, hreflang, sitemap entries, Core Web Vitals, structured data markup — are out of scope and risk conflicting advice if handled here. The boundary is clear: `blogging-content-strategy-weapon` produces the content architecture and post briefs; `seo-aeo-guardian` implements the technical substrate.

**Specific handoff triggers:**
- Schema markup (FAQPage, Article, HowTo) → `seo-aeo-guardian`
- Sitemap inclusion or priority settings → `seo-aeo-guardian`
- Core Web Vitals fixes → `seo-aeo-guardian`
- robots.txt directives → `seo-aeo-guardian`
- hreflang for multi-language → `seo-aeo-guardian`
