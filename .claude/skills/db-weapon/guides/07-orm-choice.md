...
**db-guardian's responsibility:** flag the risk in the schema/query layer with file:line, hand off to `react-guardian` for the data-layer remediation (RSC vs route loader vs TanStack Query batching vs `dataloader`-style coalesce).
...
- Hand off N+1 to `react-guardian`.
