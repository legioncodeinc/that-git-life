# Principles: crm-integration-guardian
...
There is no safe default. The four policies (last-write-wins, CRM-authoritative, product-authoritative, human-review queue) each have correct use cases. Defining the policy is the guardian's job, not the engineer's assumption. See `guides/04-sync-and-conflicts.md`.
...
If in doubt, route to `security-guardian` for GDPR compliance review. Never provide legal advice.
