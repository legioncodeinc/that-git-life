...
A phased refactor plan. Output shape when the invocation is "propose a refactor". The final artifact is handed to `library-guardian` for PRD authoring.
...
5. Replace auth-scroll-your-own → Auth.js or Clerk (involves security-guardian; out of this ADR).
...
4. Add Server Actions for mutations (guarded with Zod + auth; hand-off to `security-guardian` per action).
...
- **This plan → `library-guardian`** for PRD authoring. The PRD will expand Phase 3 and Phase 6 into user-visible milestones, acceptance criteria, risk register, rollback plan.
- **Per-phase ADRs → repo `library/architecture/ADR-<n>-<topic>.md`.**
- **Post-phase QA → `quality-guardian`.**
