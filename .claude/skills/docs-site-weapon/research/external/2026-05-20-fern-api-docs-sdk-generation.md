...
- Fern is the clear choice when the primary use case is API reference documentation + SDK distribution. It overlaps with `api-docs-guardian` scope (OpenAPI rendering) but goes further with SDK generation - document the handoff boundary clearly in the weapon.
