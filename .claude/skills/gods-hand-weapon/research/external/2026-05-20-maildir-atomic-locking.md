...
- The three-stage Maildir lifecycle (tmp -> new -> cur) is structurally identical to gods-hand's three-stage lifecycle (queue -> in-process -> completed). Document this mapping in `guides/00-principles.md`:
  - Maildir `tmp/` -> nothing direct (gods-hand doesn't have a "draft" stage; the row is born in `queue.md` by `big-bang-space`).
  - Maildir `new/` -> `proposed-angels-queue.md` (visible to all, claimable by any consumer).
  - Maildir `cur/` -> `proposed-angels-in-process.md` (exclusively owned by one consumer at a time).
  - Maildir delete-after-cur (no equivalent dir, file is in cur) -> `proposed-angels-completed.md` (archive after work is done).
- The NFS caveat (link is atomic, rename is not) does NOT apply to gods-hand because the queue files live in a single working directory on a local filesystem and the move semantics are at the agent layer (markdown editing), not at the OS layer. Document this difference in `guides/00-principles.md` so the user doesn't expect Maildir-grade NFS guarantees.
- The stale-lock recovery guidance maps directly to gods-hand's "Refuse to start a new cycle if proposed-angels-in-process.md already contains a row" rule. The in-process row IS a potential stale lock if a prior cycle crashed. `guides/10-failure-modes.md` should reference Maildir's stale-lock cleanup as prior art for the recovery procedure: examine the row, decide if work was completed (look for the artifacts on disk), then either move the row to completed.md manually or back to queue.md for a re-run.
