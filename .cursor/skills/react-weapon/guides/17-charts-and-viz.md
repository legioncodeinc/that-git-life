...
- Themed via your CSS variables (the same OKLCH tokens `ux-ui-guardian` defines).
...
Note the CSS variable references — that's how you keep the chart in lockstep with `ux-ui-guardian`'s tokens.
...
> **[Must-fix]** `src/features/dashboard/Pie.tsx:12` — colors are hex literals (`#ff6b6b`). Replace with `var(--chart-1)` etc. to inherit dark/light mode and brand changes. Hand off to `ux-ui-guardian` for token mapping.
...
- **Color, gradient, and contrast token decisions** → `ux-ui-guardian`. Every chart color is a token, never a literal.
- **Data layer (cursor pagination, downsampling, server-side aggregations)** → `db-guardian` for the API; this guide assumes data already lives in the component.
