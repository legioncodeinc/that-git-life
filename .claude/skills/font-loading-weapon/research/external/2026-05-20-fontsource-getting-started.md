...
- The docs are thin at the intro level. The weapon should link to the Fontsource variable fonts docs (not covered in the intro) for more detail on subsetting configuration.
- Key difference for weapon: Fontsource packages the font file but does NOT inline `@font-face` rules with `unicode-range` splitting. The developer is responsible for the full `@font-face` stack.
