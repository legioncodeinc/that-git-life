...
`customerId` **must** come from the authenticated user's record. Never accept it from a query string, hidden form field, or POST body. If you do, an attacker logs in as user A and creates a portal session for user B's customer ID — full subscription takeover. Surface to `security-guardian` immediately if seen.
...
This guide enforces Hard Rules #3 (never trust the client) and #10 (surface security to `security-guardian`). See `guides/00-principles.md`.
