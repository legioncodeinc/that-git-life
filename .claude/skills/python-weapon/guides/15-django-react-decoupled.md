...
5. **Auth provider choice is `auth-guardian`'s call.** This guide covers the Python wiring of session / JWT / external-token patterns.
...
When the app spans domains, includes mobile clients, or third-party API consumers. Hand the **provider choice** to `auth-guardian`. The Python wiring:
...
Refresh-token flow, MFA, RBAC, OAuth callbacks — all `auth-guardian` territory.

## Auth pattern: external provider (Clerk / WorkOS / Auth0 / Supabase Auth)

Provider SDK on the React side issues a token; Django Ninja validates via a custom `HttpBearer` that calls the provider's verify endpoint or validates the JWT against the provider's JWKS. Provider choice is `auth-guardian`; the wiring is the same shape as the JWT pattern above.
...
Discuss the React tooling choice with `react-guardian`.
...
- **Auth provider choice** → `auth-guardian`.
- **React component / state / data-fetching shape** → `react-guardian`.
- **Security audit of the auth surface** → `security-guardian`.
