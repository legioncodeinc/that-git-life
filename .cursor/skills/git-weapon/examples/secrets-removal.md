...
**Treat the credential as compromised immediately.** Escalate to `security-guardian` for credential rotation in parallel with history cleanup.
...
## Step 6: Credential rotation (escalate to security-guardian)
...
This step is `security-guardian`'s domain — escalate immediately.
