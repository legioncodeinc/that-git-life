...
- **`library-guardian`** owns storing OKR artefacts in `library/`. This Angel authors the content; `library-guardian` owns the folder structure.
- **`agile-scrum-guardian`** owns Scrum ceremonies. When sprint goals intersect with OKRs, this Angel owns the OKR side and notes the Scrum connection.
- **Domain Angels** (`react-guardian`, `db-guardian`, etc.) own the engineering work OKRs point at. This Angel writes the goal; the domain Angel plans the execution.
