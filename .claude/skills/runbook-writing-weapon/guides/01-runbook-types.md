...
Several 2026 tools (Rundeck, AWS SSM Documents, Shoreline, Jupyter notebooks with live queries) blur the line between manual runbooks and automated remediation. This weapon covers **manual runbooks only**. Automated runbooks are an extension of infrastructure-as-code owned by `devops-guardian`. If the user's organization uses runbook automation, flag the boundary: `runbook-writing-guardian` authors the human-readable procedure; `devops-guardian` implements the automation that optionally executes it.

> TODO: open question — a future `runbook-automation-guardian` could bridge this gap if demand surfaces.
...
This weapon authors **runbooks**. If the user asks for a "playbook," confirm whether they mean a specific procedure (runbook) or a strategic overview (playbook). The strategic overview is out of scope; route to `library-guardian`.
