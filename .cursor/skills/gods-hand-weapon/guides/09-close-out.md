...
## Step 10b: Delete the row from `proposed-angels-in-process.md`

Open `ai-tools/proposed-angels-in-process.md` and delete the line `NNN|guardian-name` (or `NNN|guardian-name|failed:<phase>|...` if the row carries a failure marker from a prior partial cycle; but at this point in a happy-path run, the row should be clean).
...
## Step 10c: Append the row to `proposed-angels-completed.md`

Append a single line to `ai-tools/proposed-angels-completed.md` with the canonical completed-row format:
...
If `proposed-angels-completed.md` was empty at the start of this cycle, it now contains exactly one line. Subsequent cycles append rows to the bottom.

## Step 10d: Flip the backlog checkbox to `[x]`

Open `ai-tools/proposed-angels-backlog.md` and find the entry's heading:
...
Do NOT modify anything else in the backlog entry. The four metadata lines, the Purpose, and the search queries remain unchanged. They are the historical record of the Angel's authoring inputs.
...
`gods-hand` could also touch the YAML frontmatter of `proposed-angels-completed.md` and `proposed-angels-backlog.md` to record:

- `date_updated:` (today's ISO date)
- `last_updated_by: gods-hand`

For the queue file, the frontmatter was already updated in Sub-step 2b of the pick-and-lock guide. For the in-process file, the frontmatter is optional (most cycles do not need a counted total). For the completed file, an incrementing `totals.rows` is useful for at-a-glance "how many Angels have been forged?" answers.
...
You might wonder why the close-out is not just "Phase 5" alongside the other phases. The answer: the other phases dispatch external workers. Close-out is purely `gods-hand`'s administrative work. Keeping it separate:
...
- Keeps `gods-hand`'s reporting clean (the final report enumerates four worker phases plus the close-out).
- Allows `gods-hand` to be tested with a stub worker pipeline by verifying just the close-out step.
...
- **The five artifacts exist but one is a stub.** For example, `SKILL.md` exists with only YAML frontmatter and no body. `gods-hand`'s checks should have caught this at the relevant phase boundary, but if it slips through, flag in the final report and recommend a manual re-run of the affected phase.
