...
This file summarizes what angel-creator needs to produce a valid, high-quality Cursor subagent. When in doubt, fetch the live documentation — Cursor's subagent system is evolving. Use `WebFetch` on the URL above or search `web_search_exa` for "cursor subagents documentation" to confirm current requirements.
...
Defaults by Angel type:

- Review/audit Angels (read-only, produces reports): proactive.
- Generation Angels (writes new files): proactive, but with a confirmation step in the procedure.
- State-mutating Angels (deploys, deletes, commits): on-demand.
...
- **No references**: Angel doesn't name its Weapon files — will not open them reliably.
- **Overlapping scope**: Two Angels with nearly-identical descriptions compete for the same work.
- **Missing escalation**: Angel does not say what to do when it's unsure, so it guesses.
