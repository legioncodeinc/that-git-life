...
- **Worker / queue / fan-out infrastructure** for cross-channel delivery → `devops-guardian`. The notification *worker* (cron, webhook receiver, retry logic) is theirs. The *React surface* (Inbox component, toast call sites) is ours.
- **Email template authoring (MJML, React Email)** → out of scope; flag for `library-guardian` PRD if the design team needs templates.
- **Push token storage and rotation, auth posture** → `security-guardian`.
- **Notification copy / tone / cadence policy** → `ux-ui-guardian` for copy, product for cadence.
