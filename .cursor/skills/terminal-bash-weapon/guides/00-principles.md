# Principles — terminal-bash-weapon

Core rules that govern every invocation of `terminal-bash-guardian`. Read before any other guide.
...
When the terminal context is a Docker container, Kubernetes init container, or CI runner image, the appropriate owner is `devops-guardian`, not `terminal-bash-guardian`. The difference:

- **Workstation dotfiles:** terminal-bash-guardian
- **CI step that runs `npm install`:** devops-guardian
- **Shell script that deploys to production:** devops-guardian (even if the script is Bash)
