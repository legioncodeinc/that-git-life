...
The mid-market CRM landscape in 2026 includes four platforms relevant to the `crm-integration-guardian` scope beyond HubSpot, Salesforce, and Attio: **Pipedrive** (mature, Microsoft-friendly, 500+ integrations, solid API), **Close** (inside sales / high-velocity teams, two-way email sync, built-in calling), **Copper** (Google Workspace-native, goes back 12 months on email history), and **Folk** (individual and small-team CRM, early-stage API, primarily Zapier-dependent). For production bi-directional integration work, Pipedrive and Close are viable; Folk's API is too early-stage for complex sync.
...
- **Research gap acknowledged:** This source covers folk, Close, Copper, and Pipedrive at a surface level. For a deeper Pipedrive integration guide, weapon-forge should scrape developers.pipedrive.com directly for current webhook event types, rate limits, and API v1 auth patterns.
