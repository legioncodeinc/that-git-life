...
`quality-guardian` ran 10 weeks of coaching-rubric reviews on this tenant. Findings:
...
The "cross-session connection" dimension (added by `quality-guardian` for this tenant) is below 0.5 — the coach isn't connecting current sessions to past commitments.
...
- **`db-guardian`** — verify the recursive CTE in `traverseGraph()` is using indexes (`@@index([tenantId, fromEntityId])`). Watch query plan after backfill.
- **`library-guardian`** — author the PRD if this is a paid feature.
- **`quality-guardian`** — fold the cross-session-connection rubric into the standard QA suite for this tenant.
...
PRD: handed to library-guardian
