...
After producing the policy document, identify any branch protection ruleset changes required. Route these to `github-repo-health-guardian` with the specific rule deltas — do NOT configure them yourself. The boundary is: this Angel owns the strategy; `github-repo-health-guardian` owns the GitHub/GitLab configuration UI/API.
...
| Feature flag platform selection and implementation | This Angel scopes the decision; implementation routes to `react-guardian` / `python-guardian` |
