...
Step 6 of the Command Brief's ACTION list (and an interlude between Phase 1 and Phase 2). After the Command Brief is written and the weapon folder is scaffolded, invoke the `scripture-historian` subagent to conduct depth-calibrated research.
...
2. The subagent can run with a different model than `gods-hand` (per the Command Brief's `research_model` field).
3. Research tool calls (Firecrawl, Exa) are bursty; isolating them avoids polluting `gods-hand`'s context with tool transcripts.

The other four phases (`command-center`, `weapon-forge`, `angel-creator`, `god-registrar`) are skills because their workloads are bounded, deterministic file-authoring tasks that benefit from shared context with `gods-hand`.

## Inputs `gods-hand` passes to `scripture-historian`
...
`scripture-historian`'s own contract documents the four depth tiers. As a `gods-hand` operator, you do not need to memorize them, but you should know roughly:
...
If the depth tier is `deep` or `extreme`, expect `scripture-historian` to confirm acceptance with the caller in a short pre-flight message before starting. `gods-hand` should not interfere with that exchange; it is between the subagent and the user.
...
In the early factory pipeline (before `scripture-historian` was added in late 2025), `weapon-forge` did its own research. The separation makes the pipeline more debuggable:
...
The `weapon-forge` SKILL.md still has a "Step 3: Conduct research" header from before the split, but the inline instructions defer to `scripture-historian`. See `guides/06-phase-2-weapon-forge.md` for the implication: `weapon-forge` does NOT re-run research; it consumes what is already in `research/`.
