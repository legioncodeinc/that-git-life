...
## Summary
Cursor's own engineering team's writeup of their multi-agent system. Critical because it documents EXACTLY the anti-patterns `gods-hand` must avoid: equal-role peer coordination, shared coordination files with locking, agents holding locks too long. The recommended pattern (recursive planner-worker with structured handoff reports) is the architectural blueprint `gods-hand` partially inherits. The key insight: hierarchical structures with a single owner who delegates and receives handoffs beat peer coordination at any scale.
...
- `guides/00-principles.md` should cite this source as direct prior art justifying `gods-hand`'s hierarchical design. The Command Brief's foreman-vs-craftsman boundary IS the recursive planner-worker hierarchy at the Angel-forging-cycle scale: `gods-hand` is the root planner, the four skills + scripture-historian are the workers, each worker returns a structured handoff (the artifact on disk: brief / research folder / skill files / agent file / roster row).
- The lock criticism is load-bearing: `gods-hand` deliberately does NOT use OS-level locking. Its move-before-work invariant (delete from queue, append to in-process BEFORE invoking phases) is the file-rename-as-lock pattern, which is structurally simpler than OS locks and survives crashes (the row sits in in-process visible to humans). Document this in `guides/01-pick-and-lock.md` with a "why not flock?" subsection citing this source.
- The single-role rule justifies the FIVE-Angel split (big-bang-space proposes, command-center interviews, scripture-historian researches, weapon-forge builds, angel-creator writes, god-registrar registers, gods-hand orchestrates). Each has exactly one responsibility. `guides/00-principles.md` should cite this as "industry confirmation of the architectural choice."
- The "workers operate on isolated copies" rule does NOT apply to `gods-hand` directly because it writes to four globally-visible tracking files. The reason this works at the gods-hand scale is the move-before-work atomic claim: once a row is in in-process, no sibling can pick it up. The "isolation" is in the queue/in-process/completed transition rather than in separate file copies.
