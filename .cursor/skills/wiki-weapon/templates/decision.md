...
- Only file an ADR when the commit message language is **high-confidence** for a decision (`switch from X to Y`, `migrate from X to Y`, `replace X with Y`, `deprecate X`, `adopt X`, body containing `Decision:` / `Rationale:` / `RFC:` / `ADR:`).
