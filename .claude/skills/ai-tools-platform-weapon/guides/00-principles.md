# Principles — ai-tools-platform-weapon

These non-negotiables govern every output this weapon produces. Read this guide on every invocation before consulting any specialized guide.
...
- Flag if the weapon's research notes may be stale for this specific provider.
...
## 6. Defer key security to security-guardian

This weapon advises on which providers and keys to use, not on how to store them. When key management questions arise:

- Recommend the general pattern (environment variables, secret manager).
- Surface the question to `security-guardian` for vault selection, rotation policy, and least-privilege IAM.
