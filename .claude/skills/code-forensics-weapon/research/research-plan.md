# Research Plan — code-forensics-weapon

## Context

This weapon is the generic descendant of `ada-devpipe-forensics`, which was forged on the Example Booking Co. matter (May 2026). The original investigation produced V2 and V3 master forensic packets; the calibration anchors for this Weapon's methodology come from that work.
...
- **Q:** Should the Weapon support non-WordPress marketing-site cases? **A:** Yes — the WordPress-specific CVE database and audit-log parser should be parameterized by CMS. Webflow, Shopify, Squarespace, custom-CMS cases follow the same maintenance-negligence pattern but require different evidence sources.

- **Q:** Should the Weapon include a state-court venue picker beyond Ohio? **A:** Yes — `jurisdiction-{state}.md` files for the top 10 US commercial litigation venues (Delaware, New York, California, Texas, Florida, etc.). For Example Booking Co., only Ohio was needed.
