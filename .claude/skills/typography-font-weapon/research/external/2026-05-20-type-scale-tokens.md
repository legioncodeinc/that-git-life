...
A guide covering the implementation of a type scale via CSS custom properties, including a three-tier token architecture (primitive, semantic, component tokens), Figma Tokens plugin integration, and Tailwind CSS bridging. Directly supports the font-token layer guide in the weapon.
...
- The naming conventions (`--primitive-size-*`, `--text-body-*`, `--text-heading-*`) should be the defaults in the template, but weapon-forge should note these can be adapted to match `design-system-weapon` conventions.
...
- Cross-reference with `design-system-weapon` peer weapon: the three-tier architecture is a design system concern, not a typography-only concern. This guide shows the typography-specific application of it.
