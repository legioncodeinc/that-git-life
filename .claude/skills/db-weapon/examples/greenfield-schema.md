...
-- HNSW for retrieval; cosine similarity. Hand off retrieval strategy to ai-platform-guardian.
...
| `COMMENT ON COLUMN ... IS 'PII'` | `guides/01-schema-design.md` §pii-flags — surfaces to security-guardian |
...
- [ ] PII columns commented for `security-guardian` audit.
- [ ] Supabase RLS policies designed (handed to `security-guardian`).
- [ ] PgBouncer / Supavisor in transaction mode if going serverless (`templates/pgbouncer.ini`).

## Handoffs from this schema

- `security-guardian` — audit RLS policies; verify PII flags map to retention + encryption-at-rest config.
- `react-guardian` — data-layer plan for the UI (TanStack Query / RSC / route loader).
- `ai-platform-guardian` — retrieval over `documents.embedding` (chunking, top-k, reranking).
- `quality-guardian` — verification queries after first deploy.
