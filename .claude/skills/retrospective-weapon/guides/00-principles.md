...
## The failure point is not the format

The research is clear: most retros fail at the **commitment step**, not the format step. Teams deliberate carefully about whether to use sailboat or Start/Stop/Continue, then produce seven vague action items with no owners, no deadlines, and no presence in the sprint backlog. The format is almost irrelevant if the commit step is broken.

The implication: `retrospective-guardian` prioritizes action-item discipline over format selection. A mediocre format with excellent follow-through beats a beautiful format with no follow-through.
...
Async retros are a valid first choice for many teams, not a consolation prize when schedules don't align. Research shows async retros produce 42% higher participation from introverted team members, and they often surface more thoughtful input because contributors have time to reflect. See `guides/05-async-retro.md` for the decision gate and design patterns.

## The opening ritual matters
...
Before any action item leaves the whiteboard and enters the sprint backlog, apply this filter:

1. **Who owns this?** (One name. Not "the team".)
2. **When does it close?** (A date within the next sprint.)
3. **What does done look like?** (A concrete, observable outcome.)

If an action item can't answer all three, it doesn't become a commitment. Either define it or drop it.

This filter is fast (10-15 seconds per item) and prevents the five structural failure modes: no owner, no deadline, too large, invisible on backlog, no accountability loop. See `guides/04-action-items.md` for the full action-item discipline.

---

*Cross-reference: `examples/happy-path-retro.md` shows these principles in a live facilitation.*
