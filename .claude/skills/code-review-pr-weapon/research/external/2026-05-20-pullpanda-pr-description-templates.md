...
- **"Review Focus" section** is a distinctive contribution that maps to the Angel's "reviewer hints" element in the six-element description structure. The Angel should explicitly name this element and this is the best justification for why it exists.
- **Commit messages vs. PR descriptions distinction** is a common confusion that the Angel should address directly. The clarification (commit = implementation details, PR description = human context for the overall change) belongs in the anti-patterns section of the description guide.
