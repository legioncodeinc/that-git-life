...
- **Realtime collaboration infra (Yjs server, Liveblocks, PartyKit)** → flag for `devops-guardian` or library-guardian PRD.
- **Token / typography / spacing of the editor surface** → `ux-ui-guardian`.
- **AI completions inside the editor** → `ai-platform-guardian`.
- **Server-side sanitization of editor HTML/JSON** → `security-guardian`.
