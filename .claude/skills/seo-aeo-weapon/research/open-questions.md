...
**Default in this forge:** Out of scope. `guides/00-principles.md` declares Next.js 14+ App Router scope and instructs the Angel to flag Pages Router codebases as degraded coverage, escalating to `react-guardian` for the App Router migration. Preserves the canonical playbook's scope.
...
**Default in this forge:** Always authored (playbook §9 is in scope), conditionally *applied* by the Angel. The Angel's Step 1 scoping determines whether the target is a local-business site; if not, `guides/08-local-seo.md` becomes low-priority reading rather than checklist-blocking.
...
**Default in this forge:** Documented in `research/refresh-cadence.md`. Owner left unspecified; `seo-aeo-guardian` Angel can execute the review when prompted ("refresh seo-aeo research"). The Angel re-runs the research plan's queries and updates dated notes.
