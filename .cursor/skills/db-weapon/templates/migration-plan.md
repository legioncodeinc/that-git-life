...
## Verification queries (handed to `quality-guardian`)
...
- **`quality-guardian`** — runs verification queries above; confirms green.
- **`security-guardian`** — review any new PII columns or RLS impact.
- **`react-guardian`** — flag any data-layer changes the UI must adopt.
...
*Template from `db-weapon/templates/migration-plan.md`. See `examples/zero-downtime-not-null.md` for a filled example.*
