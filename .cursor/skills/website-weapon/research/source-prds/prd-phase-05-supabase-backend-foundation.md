...
See `website-weapon/templates/rls-policy-skeleton.sql` for the complete policy set.
...
- **R-2:** Supabase's pgbouncer transaction-mode pooler (the default) may conflict with Payload's connection expectations. Test with `?pgbouncer=true&connection_limit=1` in `PAYLOAD_DATABASE_URI`. See `cms-payload-weapon/guides/04-supabase-postgres-adapter.md`.
