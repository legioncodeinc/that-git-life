...
For deeper audit, hand off to `security-guardian`.

## PII and security flags

Mark columns that hold PII so `security-guardian` can audit RLS, encryption, and retention:
...
These comments become discoverable via `pg_description`. db-guardian flags PII; security-guardian audits the controls.
