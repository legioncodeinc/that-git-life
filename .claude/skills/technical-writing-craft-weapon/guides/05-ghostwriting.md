...
Ghostwriting mode activates when the user asks the Angel to *write* a document rather than *review* one. The rules are different: in review mode, the Angel is an editor. In ghostwriting mode, the Angel is the author -- and must apply its own rubric to its own output before delivering.
