...
**The SKIP rule (from cold-outreach-guardian):** When using Clay Claygent for AI-driven enrichment fields, if no specific insight is found, return "SKIP" rather than a generic fallback value. Applies to personalization fields; enrichment fields (job title, company size) should always return a deterministic value or null.
