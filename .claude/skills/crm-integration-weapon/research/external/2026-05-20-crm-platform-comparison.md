...
- **guides/01:** Folk and Close are missing from this research pass. weapon-forge should add a shallow search specifically for these two before finalizing the scored rubric table.
- **guides/03 (Sync Architecture):** HubSpot's official docs state webhooks do NOT guarantee ordering and MAY send the same notification multiple times (see bi-directional sync source). Weapon-forge MUST encode this in the sync architecture guide as a first-class warning - handlers must be idempotent and sequence-aware using `updated_at` from the payload, not webhook delivery time.
