...
- **Three-tier tagging names** (`must-fix` / `should-fix` / `nit`) should be compared with ARDURA's (`[blocker]` / `[suggestion]` / `[nit]`) and Google's (`Nit:` / `Optional:` / `FYI:`). The Angel should standardize on one convention with a cross-reference table.
