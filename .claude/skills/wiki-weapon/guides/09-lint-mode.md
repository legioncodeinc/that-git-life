...
Per user decision 2026-04-29 on lint authority: **agent does per-chunk lint; driver does global lint.** This guide covers the agent's per-chunk responsibilities only. The driver's global pass (orphan detection, dead-link sweep across the whole wiki, ADR-chain integrity across all decisions) is documented in the Legion VS Code extension's TypeScript driver — out of scope here.
...
Ported from `gods-hand/refs/claude-obsidian-main/skills/wiki-lint/SKILL.md`, scoped to per-chunk visibility.
...
The driver aggregates per-chunk findings into `meta/<YYYY-MM-DD>-lint-report.md` with the full report shape ported from `gods-hand/refs/claude-obsidian-main/agents/wiki-lint.md`:
...
- Per-chunk catalog: ported from `gods-hand/refs/claude-obsidian-main/skills/wiki-lint/SKILL.md` and `agents/wiki-lint.md`, scoped per user decision 2026-04-29 on split lint authority.
