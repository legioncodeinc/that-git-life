...
- The distinction between checklist completion and activation is the single most important conceptual anchor for the guide. weapon-forge must call this out early as "the trap teams fall into."
...
- The "three to five items max" rule for initial checklists is an opinionated but data-backed stance weapon-forge should include as a hard recommendation, not a suggestion.
...
- Progress persistence: the guide implies localStorage for step completion state but does not prescribe a schema. weapon-forge should include a canonical schema: `{ checklistId: string, steps: Record<stepId, { completed: boolean, completedAt: ISO8601 }>, version: number }`.
- The "iteration loop that doesn't require an engineering sprint" is a Jimo product pitch, but the underlying principle (tour editable without code deploy) is a genuine architectural constraint for the weapon's build-vs-buy recommendation.
