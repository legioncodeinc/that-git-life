...
| [`codex/`](codex/) | Codex adapter docs for generating `.agents/skills`, `.codex/agents`, `.codex/hooks.json`, `AGENTS.md` guidance, and the `autopilot` profile for idea-to-merged-PR workflows. |
...
Scripts that fire automatically on session events (before or after a tool runs, on prompt submit, on session start). They make "always do X" a guarantee instead of a hope. The Codex adapter generates a project-local hook set for runtime verification and That Git Life policy warnings; the doc explains the model and how to add hooks per harness.
...
| **Hooks** | `.cursor/hooks.json` | `.claude/settings.json` | not user-configurable | `.codex/hooks.json` generated by adapter |
