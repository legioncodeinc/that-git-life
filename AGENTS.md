...
| **Codex** | `.codex/agents/*.toml` | Custom agents generated by the [Codex adapter](./codex/README.md). Codex only spawns them when explicitly asked to run subagents or parallel worker agents. |

The short version: agents are a first-class concept in Cursor and Claude Code, and they share a file format, so the same agent definition works in both. Cowork leans on skills as the portable unit, while still running agent-style delegation under the hood. Codex supports custom agents through TOML files, so the adapter translates Bee Markdown into Codex agent configuration.
