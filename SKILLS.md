...
> What skills are, how they work, and how they map across Cursor, Claude Code, Claude Cowork, and Codex.
...
In this repo, skills are called **Stingers**. Each Stinger is the procedural arsenal for one Bee (see [AGENTS.md](./AGENTS.md)). They live in [`.cursor/skills/`](./.cursor/skills/), mirrored to [`.claude/skills/`](./.claude/skills/), packaged for Cowork in [`.cowork/skills/`](./.cowork/skills/), and translated into Codex skills by the [Codex adapter](./codex/README.md).
...
Skills are the most portable asset of all. The `SKILL.md` format is shared across the supported harnesses, though Codex installs them under `.agents/skills`.
...
| **Codex** | `.agents/skills/<name>/` | Generated by `scripts/build-codex-adapter.mjs`; includes optional `agents/openai.yaml` metadata. |
