...
- The Gusto-via-Finch integration is a nuance: it's not native, so weapon-forge should note that Gusto users need to enable Finch within Carta to sync data automatically — direct integration requires Rippling or Justworks
...
- Weapon-forge should document the 409A timing rule: get the first 409A before granting any options, then refresh annually or upon a material event (funding round, acquisition discussion, 12+ months elapsed)
