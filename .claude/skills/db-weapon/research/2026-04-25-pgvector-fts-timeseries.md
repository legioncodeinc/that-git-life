...
**Hard handoff:** db-guardian picks column type, dimension (matches model), index family, and distance op. **Retrieval strategy** (top-k, hybrid dense+sparse, reranking, query expansion) is `ai-platform-guardian`'s.
...
Spine of `guides/06-special-purpose.md`. Drives hard rule #10 (handoff to `ai-platform-guardian`).
