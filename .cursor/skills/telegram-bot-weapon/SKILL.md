...
- **Do NOT own:** Mini App frontend UI/React (`react-guardian`), Dockerfile / CI for the bot server (`devops-guardian`), external payment processor details beyond Telegram Payments (`payments-guardian`).
- **Handoff rule:** when the user's question shifts from "how do I wire the Telegram side" to "how do I build the React UI inside the WebView", hand off to `react-guardian`.
