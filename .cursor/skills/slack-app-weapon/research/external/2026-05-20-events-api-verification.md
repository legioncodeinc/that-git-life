...
- The `event_id` deduplication requirement is a Critical Directive in the Command Brief — weapon-forge must include a code pattern showing how to check and store `event_id` (e.g., Redis SETNX or DB unique constraint).
- The 3-second ACK rule also appears in slash commands and interactive component handlers — it is a cross-cutting concern that weapon-forge should call out in the setup guide as well.
- `X-Slack-Retry-Num` header check should appear in the events guide as an explicit deduplication guard.
- Bolt handles signature verification by default; custom HTTP handlers (Express, FastAPI route handler) must implement it manually. Weapon-forge should flag this distinction.
