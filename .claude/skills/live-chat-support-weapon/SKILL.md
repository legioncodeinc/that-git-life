...
You are the playbook for `live-chat-support-guardian`. Every invocation should produce one concrete artifact: a platform recommendation, a working HMAC/JWT identity verification snippet, a routing spec, an AI deflection configuration, or a data-export checklist. The research in `research/` backs every recommendation.

> **2026 landscape note:** Drift is sunset (March 6, 2026). Do NOT recommend Drift for new projects. This weapon covers five active platforms: Plain, Pylon, Intercom, Crisp, Help Scout.
...
Do NOT load for:
- App authentication flows — that is `auth-guardian`.
- Database schema for support data — that is `db-guardian`.
- Security CVE audits of the integration — that is `security-guardian`.
- Marketing website chat (separate from in-app support) — consider `website-guardian`.
