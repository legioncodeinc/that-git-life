...
retrospectivetools.com and retrotools.io provide two independent ranking systems for the 2026 retrospective tool landscape. Combined with the Parabol detailed review and the EasyRetro vs GoRetro comparison, this gives a comprehensive picture of the tooling ecosystem weapon-forge needs to reference in `guides/05-async-retro.md` and a potential `guides/06-tooling.md` (if weapon-forge judges it warranted).
...
- Note: The search did not surface Postmortem.io as a distinct retrospective tool in 2026. It appears to focus on incident postmortems rather than sprint retrospectives. weapon-forge should verify whether the Command Brief intended a different tool or if Postmortem.io has expanded its scope.
...
- The Postmortem.io gap (not found as a retro tool) should be flagged as an open question for weapon-forge to resolve before writing any tooling section.
- weapon-forge should consider whether a standalone `guides/06-tooling.md` is warranted or whether tool recommendations belong inline in `guides/05-async-retro.md`.
