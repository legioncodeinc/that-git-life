...
| No rate limiting on the SSE endpoint | Medium | Add per-user rate limiting (handed to `security-guardian`) |
| Server-side URL scraping with no sandboxing — SSRF possible | High | Add allowlist or proxy through safe external service (handed to `security-guardian`) |

mind-guardian flags the SSRF concern and hands the audit to `security-guardian`.
