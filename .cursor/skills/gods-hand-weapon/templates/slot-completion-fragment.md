...
**Purpose:** Contains the exact line the orchestrator appends to `ai-tools/proposed-angels-completed.md` to record the successful forging of this Angel.
...
- `guardian-name` — kebab-case Angel name
...
3. Appends all collected lines to `ai-tools/proposed-angels-completed.md` in one write.
...
Slots that write `.failed` instead of `.done` do NOT get a completion entry. Their row stays `[ ]` in the backlog and they are excluded from the append step. They can be re-run manually with a standard gods-hand invocation.
