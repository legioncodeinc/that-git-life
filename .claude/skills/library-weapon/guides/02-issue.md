...
library/issues/backlog/ird-<###>-<kebab-slug>/
  ...
  qa/
    ird-<###>-<kebab-slug>-qa.md     QA report (authored by quality-guardian)
```
...
Create `qa/` inside the IRD folder on creation (empty). The `quality-guardian` agent writes `ird-<###>-<slug>-qa.md` there when a QA audit is requested. You own the folder structure; you never write QA content.
