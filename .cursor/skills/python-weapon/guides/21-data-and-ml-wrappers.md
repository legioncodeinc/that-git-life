...
Patterns when Python is the analytical / ML runtime. The cognitive layer (coaches, RAG, evals, vector DBs) hands off to `mind-guardian`; the Python implementation patterns underneath stay here.

## Where the boundary is

| Concern | Owner |
|---|---|
| Coach architecture, prompt cascade, RAG strategy, evals, vector DB choice, embedding model choice | **`mind-guardian`** |
| Django service hosting the cognitive code, Celery task dispatching LLM calls, FastAPI endpoint exposing AI features, Pydantic schemas at the AI boundary, httpx clients to LLM providers | **this Weapon** |
| pandas / numpy data transformation, ML model serving via FastAPI, batch vs streaming pipelines | **this Weapon** |
...
For GPU serving, batch requests, or heavy-traffic scenarios, consider Triton, vLLM, or Ray Serve — that's an ML-platform decision, surface it to `mind-guardian`.
...
The Python wiring of an LLM call (the `mind-guardian` defines the cognitive design):
...
- Behind a configurable gateway URL (the `mind-guardian` picks the gateway).
- Timed out — LLM calls can hang for a minute or more.
- Logged with token counts for cost / observability (handoff to `mind-guardian` for the AI-trace shape).
...
- This Weapon owns the Python plumbing under those designs.
