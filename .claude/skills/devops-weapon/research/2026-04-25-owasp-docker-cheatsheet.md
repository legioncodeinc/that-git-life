...
## Top recommendations relevant to this Weapon
...
(devops-weapon doesn't author the runtime invocation in cloud orchestrators, but flags this pattern in Compose.)
...
`docker run --privileged` grants the container near-host capabilities. Never used in production. devops-weapon flags any Compose `privileged: true` as Must-fix.
...
## Mapping to Weapon findings

| Cheatsheet rule | Weapon finding location |
|---|---|
...
This document is the canonical reference for the security-side of Dockerfile authoring. Every Must-fix Dockerfile finding traces to a cheatsheet rule. Hand off CVE deep audits to `security-guardian`; devops-guardian enforces the structural rules.
