...
This Weapon encodes the 2026 state-of-the-art for the full technical typographic stack: font loading strategy, variable font configuration, fluid type scale arithmetic, vertical rhythm, and the font-token layer. Read `SKILL.md` for task routing; follow the specific guide for implementation depth.
...
|----------|------------------------|
| Font loading strategy (next/font, Fontsource, self-host) | Typeface aesthetic selection - `design-system-guardian` |
| Variable font axes, subsetting, `@font-face` config | Build pipeline subsetting CI steps - `devops-guardian` |
| `font-display` semantics and FOIT/FOUT/FOFT remediation | Per-component type token application - `ux-ui-guardian` |
| Fluid `clamp()` type scale arithmetic | LCP font impact analysis in broader CWV audit - `seo-aeo-guardian` |
| Vertical rhythm and `line-height` tokens | Persisted user font preference schema - `db-guardian` |
| Font-token CSS layer (`--font-*` custom properties) | Brand identity and palette decisions - `design-system-guardian` |
