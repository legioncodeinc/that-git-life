...
- **Enforcement mechanic**: "CI check that flags, doesn't hard-block" is the correct implementation pattern. The Angel's `guides/03-small-prs.md` should specify this as the recommended enforcement approach.
...
- **Contradiction**: This source frames AI review as a "pre-screening step" before human review. The Angel's scope is human review culture, so AI tooling is context but not the Angel's primary focus. Weapon-forge should not overweight the AI tooling angle.
