...
Three stub scanners react-weapon can invoke on a codebase. All are written in TypeScript; run with `pnpm tsx` (or `npx tsx`).
...
Checks built bundle sizes against per-route budgets from `react-weapon.budgets.json`. Fails CI on violation unless a waiver is recorded.
