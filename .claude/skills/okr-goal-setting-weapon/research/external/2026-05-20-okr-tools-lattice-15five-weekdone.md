...
- Per the Command Brief critical directive: the weapon should advise on WHAT to configure (field mapping, cycle setup, check-in workflow) but direct users to the tool's current documentation for WHERE the settings are in the UI - tool UXs change frequently.
