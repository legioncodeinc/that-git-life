...
> **[Must-fix]** `src/server/upload.ts:1` — file size + mime allowlist enforced only client-side. Re-enforce on the server. See `guides/11-server-components.md §server-actions` and `security-guardian` handoff.

---

## Handoffs

- **Storage layer (S3 / R2 / GCS), signed URLs, lifecycle policies** → `devops-guardian`.
- **Auth scoping per upload (who can write where)** → `security-guardian`.
- **Image transforms / CDN delivery** → image-CDN territory (Cloudinary, Imgix, unpic). Out of scope for this guide; flag for library-guardian PRD if needed.
- **Visual treatment of drop zones, progress, and tree rows** → `ux-ui-guardian`.
