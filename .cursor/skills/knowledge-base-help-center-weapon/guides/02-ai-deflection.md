...
**Architecture boundary:** This pattern requires `mind-guardian` for implementation. `knowledge-base-help-center-guardian` specifies the KB export format and chunking strategy inputs; `mind-guardian` implements the embedding model, vector store (Qdrant), re-ranking (Cohere rerank-v3.5), and retrieval API.

**Hand-off protocol to `mind-guardian`:**
...
| Custom product UI, maximum retrieval control | C: Custom RAG → hand off to `mind-guardian` |
