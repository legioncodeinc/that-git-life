...
The non-negotiable rules that govern every `gods-hand` invocation. Read this guide on every cycle; the rest of the weapon assumes you have internalized these principles.

## The foreman vs craftsman boundary

`gods-hand` is the factory foreman. It does not research, design, code, audit, or author any product-domain content. Its job is to dispatch the worker phases in the correct order, watch each phase report success, and update the tracking files between phases.
...
The five-Angel-plus-orchestrator split (big-bang-space proposes, command-center interviews, scripture-historian researches, weapon-forge builds, angel-creator writes, god-registrar registers, gods-hand orchestrates) is the implementation of this rule.
...
> Before invoking ANY phase, the queue row being processed must already be deleted from `proposed-angels-queue.md` AND appended to `proposed-angels-in-process.md`.

The invariant exists because there is no OS-level lock on a markdown file. If two `gods-hand` invocations were to read the queue concurrently, both would see the same top row. The move-before-work pattern is the agent-layer equivalent of an atomic rename: by the time phase work begins, the row is verifiably in `in-process` and not in `queue`, so a second invocation entering at the same moment would see a different top row OR refuse to start (because the in-process file is non-empty).
...
`gods-hand`'s markdown-row move is the moral equivalent. The semantics are the same: claim before work, never work before claim.
...
`gods-hand` is the root of a planner-worker hierarchy. The worker phases never talk to each other directly. Each worker hands its output back to `gods-hand` via the on-disk artifact (Command Brief, research folder, weapon folder, Angel file, roster row). `gods-hand` is the only entity that reads multiple workers' outputs.
...
3. Adding a sixth phase (or removing one) only changes `gods-hand`'s phase list, not the workers.
...
For `gods-hand`'s single-cycle scope, "handoff reports" are the artifacts on disk.

## Markdown files as a state machine

The four tracking files are not loose documentation. They are a finite state machine encoded in markdown. Each Angel proposal has exactly four observable states:

| State | Where the row lives |
|---|---|
| Proposed | `proposed-angels-backlog.md` (with `### [ ] N. name`) AND `proposed-angels-queue.md` (with `NNN\|name`) |
| In-flight | `proposed-angels-backlog.md` (still `[ ]`) AND `proposed-angels-in-process.md` (with `NNN\|name`) |
| Completed | `proposed-angels-backlog.md` (now `[x]`) AND `proposed-angels-completed.md` (with `NNN\|name\|completed\|YYYY-MM-DD\|...`) |
| Failed mid-cycle | `proposed-angels-backlog.md` (still `[ ]`) AND `proposed-angels-in-process.md` (with `NNN\|name\|failed:<phase>`) |
...
The tracking files ARE the checkpoint files. `gods-hand` survives context compaction by re-reading them.
...
7. **Stop after god-registrar.** Emit the final report, then end the run. Do not loop.
8. **Refuse to start a new cycle if `proposed-angels-in-process.md` already contains a non-blank row.**
